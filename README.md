...
Claude Desktop에 입력:
```
마드리드에서 2026년 6월 11일~12일 1박 숙소를 검색해줘.
가격은 원화(KRW)로 보여주고, 리뷰 점수, 위치, Booking.com 예약 링크를 포함해줘.
```

**정상 동작 확인:**
- 호텔 이름, **원화(KRW) 가격**, 리뷰 점수(/10), 위치가 표시되는가
- **Booking.com 예약 링크**가 포함되는가 (호텔 상세 조회 시)
- 최소 5개 이상 호텔이 나오는가
...
```
바르셀로나 관광지를 검색해줘.
입장료, 리뷰 평점, 무료 취소 가능 여부를 포함해서 보여줘.
```

**정상 동작 확인:**
- 관광지 이름, 가격(USD), 평점(/5), 리뷰 수가 표시되는가
- "Sagrada Família" 등 유명 관광지가 포함되는가
- 무료 취소 가능 여부(Yes/No)가 표시되는가

### 테스트 3: 관광지 상세

테스트 2 결과를 보고 Claude Desktop에 입력:
```
사그라다 파밀리아 입장권 상세 정보를 알려줘.
주소, 포함/미포함 사항, 취소 정책, 오디오 가이드 지원 언어를 보여줘.
```

**정상 동작 확인:**
- 정확한 주소(Carrer de la Marina, 253)가 표시되는가
- 미포함 사항(가이드, 타워 입장 등)이 나오는가
- 오디오 가이드 지원 언어에 Korean이 포함되는가
...
```
"리스본 6/7~9 숙소 중 리뷰 9점 이상만 원화로 추천해줘. 예약 링크도 포함해줘."
"그라나다에서 할 수 있는 투어 검색하고, 무료 취소 가능한 것만 보여줘"
"포르투에서 리스본 가는 6월 10일 항공편 찾아줘"
"바르셀로나 가우디 관련 관광지만 모아서 가격, 리뷰, 예약 링크 포함해서 정리해줘"
```

## 제공 도구 (8개)

### 숙소 검색
- **search_destinations** — 도시/목적지 ID 검색
- **get_hotels** — 호텔 목록 (가격, 리뷰, 할인율)
- **get_hotel_details** — 호텔 상세 (시설, 체크인/아웃, 설명)

### 항공편 검색
- **search_flight_destinations** — 공항/도시 ID 검색
- **search_flights** — 항공편 목록 (가격, 경유, 수하물)

### 관광지 검색
- **search_attraction_locations** — 관광지 위치 ID 검색
